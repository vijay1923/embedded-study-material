...
int main()
{
        ...
        while(1)
        {
                ...
                if (num_bytes < 0)
		 {
                ...
                read1[i]='\0';
                printf("%s\n",read1);
                for(j=0,i=13;i<21;i++,j++)
                        t[j]=read_buf[i];
                t[j]='\0';
                for(k=0,i=22;i<30;i++,k++)
                        date[k]=read_buf[i];
                date[k]='\0';
                int c;
                char s1[50],s2[50];
                ...
                while(fgets(s1,50,fp))
                {
                        ...
                        if(strstr(s1,read1))
                        {
                                ...
                                if(c%2==0)
                                        fprintf(fd,"%s IN TIME %s %s\n",s1,t,date);
                                else
                                        fprintf(fd,"%s OUT TIME %s %s\n",s1,t,date);
                                fclose(fd);
                                rewind(fd);
                                printf("card scanned successfully\n");
                        }
                }
                rewind(fp);
        }
